...
const ChatRoom = () => {
  ...
  const findMessages = () => {
    ...
    axios
      .get(
        `${apiUrl}/api/chat?teamId=${id}&lastTimeStamp=${
      ...
      .then((response) => {
        ...
        lastTimeStampRef.current =
          response.data.items[0]?.sentDate || lastTimeStampRef.current;
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  useEffect(() => {
    ...
    switch (lastJsonMessage.type) {
      ...
      case "delete":
        setMessages((prev) =>
          prev.filter((msg) => msg.messageId !== lastJsonMessage.messageId)
        );
        break;
      default:
      ...
      <form className="input-message">
        <div className="textarea-wrapper">
          <TextareaAutosize
            ...
            onKeyDown={(e) =>
              e.ctrlKey && e.key === "Enter" && sendMessageHandler()
            }
          />
          <button
            type="button"
